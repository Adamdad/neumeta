...
def setup_evaluation_data(batch_size=128):
    ...
    # Compose the transformations applied to the test images
    ...
    train_dataset = datasets.CIFAR10(root='./data', train=True, transform=transform_test, download=True)
...
def main():
    ...
    for imp_name, imp in imp_dict.items():
        ...
        with open(f"pruning_results_{model_name}_{imp_name}.txt", "w") as results_file:
            ...
            for pruning_ratio in [i * 0.05 for i in range(20)]:  # Adjust the range/sequence as needed
                ...
                model = create_model_cifar10(model_name=model_name, hidden_dim=64).to(device)
